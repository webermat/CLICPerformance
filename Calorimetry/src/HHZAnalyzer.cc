...
HHZAnalyzer aHHZAnalyzer;

//HHZAnalyzer::HHZAnalyzer() : Processor("HHZAnalyzer"),_fju(new FastJetUtil()) 
HHZAnalyzer::HHZAnalyzer() : Processor("HHZAnalyzer") 
{


    // modify processor description
    _description = "HHZAnalyzer calculates properties of calorimeter showers" ;
   
    registerProcessorParameter( "OutputRootFileName",
                                "ROOT File name to collect plots",
                                m_rootFileName,
                                std::string("HHZAnalyzer.root"));
...
void HHZAnalyzer::init() {

  std::cout<<"R/beta/gamma parameter HHZAnalyzer "<<m_R<<"/"<<m_beta<<"/"<<m_gamma<<std::endl;
  ...
  m_trueME_Charge= new std::vector<int>();
  ...
  */
  ...
  if(m_saveMEInfo){
    ...
    m_outputTree->Branch("trueME_Charge", "std::vector< int >", &m_trueME_Charge);
...
void HHZAnalyzer::processRunHeader( LCRunHeader*) {

}

void HHZAnalyzer::processEvent( LCEvent* evt ) { 
  m_eventCount+=1;
  ...
  */
  ...
  if( mcColl != 0 ){
    ...
    for(int m=0;m<nMCP;m++){
      ...
      //index 8 is Higgs,
      //index 9 is Higgs --> H1+1
      //index 10 is Z --> H1+2
      //then fill daughters from H1 --> H1+3, H1+4
	...
	if(mcp->getDaughters().size()>0){
	  m_trueME_PDGID_D1->push_back(mcp->getDaughters()[0]->getPDG());
	}else{
	  m_trueME_PDGID_D1->push_back(0);
	}
	m_trueME_ParentID->push_back(0);
	m_trueME_Charge->push_back(mcp->getCharge());
      }
	...
	m_trueME_E->push_back(mcp2->getDaughters()[0]->getEnergy());
	m_trueME_Px->push_back(mcp2->getDaughters()[0]->getMomentum()[0]);
	m_trueME_Py->push_back(mcp2->getDaughters()[0]->getMomentum()[1]);
	m_trueME_Pz->push_back(mcp2->getDaughters()[0]->getMomentum()[2]);
	m_trueME_PDGID->push_back(mcp2->getDaughters()[0]->getPDG());
	m_trueME_PDGID_D1->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getPDG());
	m_trueME_ParentID->push_back(1);
	m_trueME_Charge->push_back(mcp2->getDaughters()[0]->getCharge());

	m_trueME_E->push_back(mcp2->getDaughters()[1]->getEnergy());
	m_trueME_Px->push_back(mcp2->getDaughters()[1]->getMomentum()[0]);
	m_trueME_Py->push_back(mcp2->getDaughters()[1]->getMomentum()[1]);
	m_trueME_Pz->push_back(mcp2->getDaughters()[1]->getMomentum()[2]);
	m_trueME_PDGID->push_back(mcp2->getDaughters()[1]->getPDG());
	m_trueME_PDGID_D1->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getPDG());
	m_trueME_ParentID->push_back(1);
	m_trueME_Charge->push_back(mcp2->getDaughters()[1]->getCharge());
	//mcp 3 should be H2
	MCParticle *mcp3 = static_cast<MCParticle*>(mcColl->getElementAt(9));
	m_trueME_E->push_back(mcp3->getDaughters()[0]->getEnergy());
	m_trueME_Px->push_back(mcp3->getDaughters()[0]->getMomentum()[0]);
	m_trueME_Py->push_back(mcp3->getDaughters()[0]->getMomentum()[1]);
	m_trueME_Pz->push_back(mcp3->getDaughters()[0]->getMomentum()[2]);
	m_trueME_PDGID->push_back(mcp3->getDaughters()[0]->getPDG());
	m_trueME_PDGID_D1->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getPDG());
	m_trueME_ParentID->push_back(2);
	m_trueME_Charge->push_back(mcp3->getDaughters()[0]->getCharge());

	m_trueME_E->push_back(mcp3->getDaughters()[1]->getEnergy());
	m_trueME_Px->push_back(mcp3->getDaughters()[1]->getMomentum()[0]);
	m_trueME_Py->push_back(mcp3->getDaughters()[1]->getMomentum()[1]);
	m_trueME_Pz->push_back(mcp3->getDaughters()[1]->getMomentum()[2]);
	m_trueME_PDGID->push_back(mcp3->getDaughters()[1]->getPDG());
	m_trueME_PDGID_D1->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getPDG());
	m_trueME_ParentID->push_back(2);
	m_trueME_Charge->push_back(mcp3->getDaughters()[1]->getCharge());
	MCParticle *mcp4 = static_cast<MCParticle*>(mcColl->getElementAt(10));
	m_trueME_E->push_back(mcp4->getDaughters()[0]->getEnergy());
	m_trueME_Px->push_back(mcp4->getDaughters()[0]->getMomentum()[0]);
	m_trueME_Py->push_back(mcp4->getDaughters()[0]->getMomentum()[1]);
	m_trueME_Pz->push_back(mcp4->getDaughters()[0]->getMomentum()[2]);
	m_trueME_PDGID->push_back(mcp4->getDaughters()[0]->getPDG());
	m_trueME_PDGID_D1->push_back(mcp4->getDaughters()[0]->getDaughters()[0]->getPDG());
	m_trueME_ParentID->push_back(3);
	m_trueME_Charge->push_back(mcp4->getDaughters()[0]->getCharge());

	m_trueME_E->push_back(mcp4->getDaughters()[1]->getEnergy());
	m_trueME_Px->push_back(mcp4->getDaughters()[1]->getMomentum()[0]);
	m_trueME_Py->push_back(mcp4->getDaughters()[1]->getMomentum()[1]);
	m_trueME_Pz->push_back(mcp4->getDaughters()[1]->getMomentum()[2]);
	m_trueME_PDGID->push_back(mcp4->getDaughters()[1]->getPDG());
	m_trueME_PDGID_D1->push_back(mcp4->getDaughters()[1]->getDaughters()[0]->getPDG());
	m_trueME_ParentID->push_back(3);
	m_trueME_Charge->push_back(mcp4->getDaughters()[1]->getCharge());
	if(abs(mcp2->getDaughters()[0]->getPDG())==24 || abs(mcp2->getDaughters()[0]->getPDG())==23){
	  //daughters of first vector boson from first H boson
	  m_trueME_E->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getEnergy());
	  m_trueME_Px->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(4);
	  m_trueME_Charge->push_back(mcp2->getDaughters()[0]->getDaughters()[0]->getCharge());	  
	  m_trueME_E->push_back(mcp2->getDaughters()[0]->getDaughters()[1]->getEnergy());
	  m_trueME_Px->push_back(mcp2->getDaughters()[0]->getDaughters()[1]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp2->getDaughters()[0]->getDaughters()[1]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp2->getDaughters()[0]->getDaughters()[1]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp2->getDaughters()[0]->getDaughters()[1]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp2->getDaughters()[0]->getDaughters()[1]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(4);
	  m_trueME_Charge->push_back(mcp2->getDaughters()[0]->getDaughters()[1]->getCharge());
	  //daughters of second vector boson from first H boson
	  m_trueME_E->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getEnergy());
	  m_trueME_Px->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(5);
	  m_trueME_Charge->push_back(mcp2->getDaughters()[1]->getDaughters()[0]->getCharge());	  
	  m_trueME_E->push_back(mcp2->getDaughters()[1]->getDaughters()[1]->getEnergy());
	  m_trueME_Px->push_back(mcp2->getDaughters()[1]->getDaughters()[1]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp2->getDaughters()[1]->getDaughters()[1]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp2->getDaughters()[1]->getDaughters()[1]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp2->getDaughters()[1]->getDaughters()[1]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp2->getDaughters()[1]->getDaughters()[1]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(5);
	  m_trueME_Charge->push_back(mcp2->getDaughters()[1]->getDaughters()[1]->getCharge());
	}
	if(abs(mcp3->getDaughters()[0]->getPDG())==24 || abs(mcp3->getDaughters()[0]->getPDG())==23){
	  //daughters of first vector boson from second H boson
	  m_trueME_E->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getEnergy());
	  m_trueME_Px->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(6);
	  m_trueME_Charge->push_back(mcp3->getDaughters()[0]->getDaughters()[0]->getCharge());	  
	  m_trueME_E->push_back(mcp3->getDaughters()[0]->getDaughters()[1]->getEnergy());
	  m_trueME_Px->push_back(mcp3->getDaughters()[0]->getDaughters()[1]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp3->getDaughters()[0]->getDaughters()[1]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp3->getDaughters()[0]->getDaughters()[1]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp3->getDaughters()[0]->getDaughters()[1]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp3->getDaughters()[0]->getDaughters()[1]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(6);
	  m_trueME_Charge->push_back(mcp3->getDaughters()[0]->getDaughters()[1]->getCharge());
	  //daughters of second vector boson from second H boson
	  m_trueME_E->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getEnergy());
	  m_trueME_Px->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(7);
	  m_trueME_Charge->push_back(mcp3->getDaughters()[1]->getDaughters()[0]->getCharge());	  
	  m_trueME_E->push_back(mcp3->getDaughters()[1]->getDaughters()[1]->getEnergy());
	  m_trueME_Px->push_back(mcp3->getDaughters()[1]->getDaughters()[1]->getMomentum()[0]);
	  m_trueME_Py->push_back(mcp3->getDaughters()[1]->getDaughters()[1]->getMomentum()[1]);
	  m_trueME_Pz->push_back(mcp3->getDaughters()[1]->getDaughters()[1]->getMomentum()[2]);
	  m_trueME_PDGID->push_back(mcp3->getDaughters()[1]->getDaughters()[1]->getPDG());
	  m_trueME_PDGID_D1->push_back(mcp3->getDaughters()[1]->getDaughters()[1]->getDaughters()[0]->getPDG());
	  m_trueME_ParentID->push_back(7);
	  m_trueME_Charge->push_back(mcp3->getDaughters()[1]->getDaughters()[1]->getCharge());
	}
      }
      if(m==4 || m==5){
  ...
  if(particleMCJetIn->getNumberOfElements()>1){
    ...
    PseudoJetList genjets = _csgen->exclusive_jets((int)(3));
 ...
 if(recorefJet_4jets_Coll->getNumberOfElements()>0){
  ...
  // get index number for flavor tagging
	...
	//std::cout<<"HHZAnalzyer "<<particleRECOJetIn->getNumberOfElements()<<" size of jet "<<_csreco_rfj_4jets->constituents((*recojetIt)).size()<<std::endl;
 ...
 */
  ...
  */
  ...
  if(particleRECOJetIn->getNumberOfElements()>1){
    ...
    PseudoJetList recojets = _csreco->exclusive_jets((int)(3));
    ...
    unsigned int recojet_index_=0;
    for (std::vector<fastjet::PseudoJet>::iterator recojetIt = recojets.begin(); recojetIt != recojets.end(); ++recojetIt ) {
      //std::cout<<"HHZAnalzyer "<<particleRECOJetIn->getNumberOfElements()<<" size of jet "<<_csreco->constituents((*recojetIt)).size()<<std::endl;
	...
	//std::cout<<"HHZAnalyzer cluster subjet"<<std::endl;
...
void HHZAnalyzer::check(LCEvent*){
}

void HHZAnalyzer::end(){
...
void HHZAnalyzer::getCollection(LCCollection* &collection, std::string collectionName, LCEvent* evt){
  try{
...
void HHZAnalyzer::fillStableDaughterSet(MCParticle* mcp, std::set<MCParticle*> &stableDaughterSet){
  if(mcp->getGeneratorStatus()==1){
