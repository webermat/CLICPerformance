#ifndef HHZAnalyzer_h
#define HHZAnalyzer_h 1
...
class HHZAnalyzer : public Processor {
                
public:
        
    virtual Processor*  newProcessor() { return new HHZAnalyzer ; }
    
    HHZAnalyzer() ;

    HHZAnalyzer(const HHZAnalyzer&) = delete;
    HHZAnalyzer& operator=(const HHZAnalyzer&) = delete;
...
protected:
    ...
    std::vector<int>* m_trueME_Charge=NULL;
    //0 if particle has no daughters
    std::vector<int>* m_trueME_PDGID_D1=NULL;
    //0 if parent not saved, 1 for H1, 2 for H2, 3 for Z, 4 for daughter1 of H1, 5 for d2 of H1, 6 for d1 of H2, 7 for d2 of H2
